# 研究与分析

`docs/research/` 保存外部资料的分析过程、可迁移原则、项目转译理由与明确排除项。它回答“这些来源能支持怎样的判断”，不直接定义当前产品行为，也不保存需要逐字保持的官方正文。

## 当前记录

- [`archive-period-design.md`](archive-period-design.md)：早期文档式网页的抽象原则、旧剧场名录转译与明确排除项；
- [`design-and-service.md`](design-and-service.md)：文化机构网站的信息架构、服务能力和泰拉化转译；
- [`ticketing-gameplay.md`](ticketing-gameplay.md)：重复体验、现实票务、可访问性与《明日方舟》玩法语境分析；
- [`ticketing-platform-logo-study.md`](ticketing-platform-logo-study.md)：现实票务平台 Logo 的视觉拆解，以及水稻网与跳楼机正式原创 SVG 的转译依据；
- [`victorian-visual-system.md`](victorian-visual-system.md)：维多利亚设计改革、剧场印刷、现实机构和官方活动语境的视觉分析；
- [`browser-compatibility.md`](browser-compatibility.md)：跨引擎测试能力、渲染性能判断和 Safari 代理边界。

## 维护规则

- 记录公开链接、访问或记录日期、事实观察、分析推断、项目转译和排除项；不得把五者写成同一证据层级；
- 官方页面可以作为研究来源被引用，但官方逐字正文和素材权利记录应进入 `docs/sources/`；
- 研究结论只有进入 active 蓝图、项目作者资料或源码后才获得相应效力。本目录不维护“当前已经实现”的第二份清单；
- 当前实现变化时优先修正正式蓝图、架构或源码，不为保持研究文字同步而复制路径、参数、页面数量和测试结果；
- 不保存无使用权的第三方图片、字体、音频、视频、代码或独特页面构图。
