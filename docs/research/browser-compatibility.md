...
研究支持“跨引擎复用断言”“按帧预算定位重绘”“减少大面积滤镜、模糊阴影和长页面伪元素”“将无语义装饰合并为单一原创环境资产”等抽象原则。项目不复制浏览器厂商示例页面、调试器界面、代码或图像，也不根据 UA 字符串隐藏效果、不为 Firefox/Safari 提供低强度污染，不把 `content-visibility` 当成未经测量的通用修复。实际命令与当前测试范围只在 `docs/guides/development.md` 维护。
