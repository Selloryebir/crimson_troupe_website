...
首轮基线在 `dev_experiment_terra_terminology` 尚未吸收技术优化时建立。准备 PR 时已按正常 merge 同步 `dev_code`，保留主线图片、日期格式、搜索降级与无障碍修复，并复核其与本分支流程的兼容性。当前记录对应同步后的内容；技术优化没有改写场次、地点、主创与项目前提，不重复记作本分支新增创意或历史缺陷。
