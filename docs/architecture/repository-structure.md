...
当前源码已经完成 S1 路由与领域骨架：`src/pages/index.astro` 只负责进入 `/yan/`，表站与 1091 里站使用独立页面树；`src/data/performances.ts` 与 `src/data/productions.ts` 分别保存场次和剧目事实；当前最小页面不需要客户端 JavaScript。
...
- 搜索页按国家版本、世界和快照年份隔离；当前明确显示索引尚未实现，不伪造结果；
- 票务页使用稳定 URL，当前分别退化为表站“暂未开票”和里站历史不可购买状态。

后续阶段继续实现完整内容、双站视觉、构建期搜索索引、污染与票务增强；不得恢复单页锚点、混合模型或双世界同页 DOM 作为第二套正式架构。
...
后续仅在出现真实复用时增加 `front/`、`archive/` 或其他能力目录；覆盖呈现不得承担本应独立路由的完整页面。
...
当前 S1 页面没有必须由 JavaScript 才能完成的任务，因此不加载空客户端入口。新增交互时，每个页面须有明确装配入口，或由单一入口按页面能力安全初始化；页面没有某个模组根节点时直接跳过。
...
`src/styles/main.css` 当前承载 S1 共享基础、表站、里站和响应式样式。规则规模或加载边界产生实际收益时再按页面或能力拆分，并继续保持以下责任：
