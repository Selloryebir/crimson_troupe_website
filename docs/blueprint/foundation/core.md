...
- 表站时间只定义为叙事语义上的“现在”，不使用影响界面、路由或构建的固定年份；
- 里站快照年份固定为泰拉历 1091 年，并通过 `/archive/site/1091/` 路由段表达；
- `archive` 只表示里站时间层，不承担“历史演出”栏目语义；
- 默认正式构建只生成通过发布门禁的炎国 `yan / zh-CN` 页面；获准的开发预览可以额外生成本地化蓝图明确列入当前预览集合的国家版本，但不因此改变正式入口或发布资格。本蓝图不复制预览版本清单。
...
- 正式构建与开发预览分别只生成各自范围内的页面，未知构建模式在页面生成前失败；
