...
本文件是当前 13 项活页剧目标题形式、官方简体中文描述及项目稳定 `productionId` 映射的唯一长期参考。标题和描述由项目负责人提供并人工确认，来源定位为《傀影与猩红孤钻》相关活页剧目；描述于 2026-08-29 固定，本次迁移不重新核验外部页面。后续若发现来源差异，先保留现有稳定 ID 与原记录，并交由人工确认修正，不静默覆盖。
...
- 标题与描述记录不自动生成体裁、主创、场次、搜索结果或详情页。运行时只为当前实际采用的剧目维护完整国家版本内容；若选用官方描述，必须与概念站编演说明明确区分，并按本地化契约同步全部构建范围；
- `folio` 只表示采用人工确认活页来源的剧目。没有直接取自本文件的简介、主创、场馆和演出记录仍是概念站创作，不得描述为活动已经确认的事实；
