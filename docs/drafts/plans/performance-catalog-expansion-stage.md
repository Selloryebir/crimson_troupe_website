...
| `ART-01`     | 已批准待同步 | 七项即将公开的剧目各有独立无文字 WebP、Artwork 元数据、摘要和权利记录；仍不改变根集合                 | `CATALOG-01` 已提交、推送并同步回 `dev_blueprint` | `src/assets/images/productions/`、`src/data/production-artwork-assets.ts`、`src/data/production-artwork-manifest.ts`、`src/data/production-artworks.ts`、资产验证  | 图像可解码且宽高明确；七项 ID 各有唯一资产和焦点／裁切／记忆色／污染参数；当前产物不泄漏集合外资产；人工参考原图不进入仓库                                         |
