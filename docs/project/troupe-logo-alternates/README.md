...
网站当前采用“小标识默认哑光、首页大幅展示显式金属”的双母版规则；本目录中的历史 A／D 环境变体不因此获得消费位置。本目录不参与构建，页面和构建器不得直接读取这些文件；项目负责人另行选定变体后，应将唯一运行时副本迁入 `src/assets/brand/`，再由对应组件显式引用。
...
- 禁止：不得因表站、里站、污染等级或页面位置自动替换当前获准的运行时母版；不得由构建器或页面直接读取本目录。
...
- 状态：已保留，非默认，暂无运行时消费者；
- 视觉：相较制作时的 D 母版提高约 22% 的可见像素加权亮度，金属层次更清楚，红色中轴更醒目；
...
- 状态：已保留，非默认，暂无运行时消费者；
- 视觉：相较制作时的 D 母版提高约 41% 的可见像素加权亮度，旧金趋向仪式性高光，外围暖光最强；
