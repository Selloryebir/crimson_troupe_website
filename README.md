...
表站与里站共享工程基础，但使用独立 URL、页面装配和内容节奏，不是同一页面的明暗换肤。正式构建当前生成 32 个静态页面（含根路径入口与炎国简体中文双站）；三语言预览生成 94 个页面并加入东国日语、哥伦比亚美式英语等价页面。两种构建均包含独立场次与剧目、分域搜索、四级污染和表站模拟购票体验。
...
需要检查炎国、东国与哥伦比亚三语言架构时，使用 `npm run dev:preview` 或 `npm run build:preview`。预览构建不改变正式发布集合；构建后使用 `npm run validate:build:preview` 检查 94 个页面的路由、元数据和搜索隔离。
...
当前候选已经实现根路径跳转、炎国版表站与 1091 里站独立页面树、`Production` / `Performance` 领域数据、双站独立视觉语法、分域搜索、四级污染及带存单和逐场纪念票的模拟购票体验。表站与等级 `0` 里站的核心内容不依赖客户端 JavaScript；语义状态不保存显示语言，状态与静态产物具有可重复验证门禁。正式构建只发布 `yan / zh-CN`；`higashi / ja-JP` 与 `columbia / en-US` 仅进入三语言技术预览，尚未经过人工翻译审核，不标记为 `formal`。
