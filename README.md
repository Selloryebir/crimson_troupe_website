...
当前候选已经实现根路径跳转、炎国版表站与 1091 里站独立页面树、`Production` / `Performance` 领域数据、双站独立视觉语法、分域搜索、四级污染及带存单和逐场纪念票的模拟购票体验。表站与等级 `0` 里站的核心内容不依赖客户端 JavaScript；语义状态不保存显示语言，状态与静态产物具有可重复验证门禁。默认展示只生成 `yan / zh-CN`；`higashi / ja-JP` 与 `columbia / en-US` 仅进入三语言技术预览，尚未经过人工翻译审核。当前三种国家版本内容均为运行时预览，不标记为 `formal`，显式正式构建尚不可用。
