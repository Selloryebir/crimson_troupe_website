...
演出创意新增、修改或删除后，智能体主动进行年鉴、设定、自洽性和多语言审查，不需逐次请求授权。`npm run review:creative:plan` 定位变化，`quality`（包括限定路径）／`verify` 检查全局输入对应的当前记录；内容与依据未变时直接复用。AI 译文用于预览，等待人工考据替换。替换入口、表里站与未来快照边界见[演出创意审查与替换](docs/guides/creative-content-review.md)，当前发现见[审查研究入口](docs/research/creative-content-review/README.md)。
...
本项目为个人学习与同人创作概念，不代表《明日方舟》或其版权方。运行时使用了项目负责人确认的官方活页剧目标题与简体中文描述，以及依据本地授权参考重新实现的国家徽章轮廓；不直接打包游戏官方原始低清图片、视频、音频、字体或代码。里站资源另含由官方参考图修复出的普通版与猩红版高清活页封面各 13 张：当前 8 个活页剧目在污染等级 0 使用普通版，等级 1 按会话抽取约三分之一使用对应猩红版，等级 2 全部对应猩红版，等级 3 全部收束为猩红版《摇篮曲》。这些图像不是项目原创；项目负责人已允许上传公开仓库，权利方公开使用与再分发许可仍未核实，不据此宣称正式发布资格。其他视觉由项目原创 HTML/CSS、SVG 与生成图像构成，具体来源和权利边界见 [`docs/sources/`](docs/sources/README.md)。
