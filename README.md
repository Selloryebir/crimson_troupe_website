...
表站与里站共享工程基础，但使用独立 URL、页面装配和内容节奏，不是同一页面的明暗换肤。当前源码已经建立炎国版双站静态页面与领域骨架；完整内容、最终视觉、搜索、污染和票务互动仍按后续阶段开发。
...
当前 S1 基线已经实现根路径跳转、炎国版表站与 1091 里站的独立路由、`Production` / `Performance` 领域数据、真实导航、页面元数据以及搜索与票务的明确降级页。页面核心任务不依赖客户端 JavaScript。完整表站内容与视觉、1091 内容与年代视觉、四级污染、正式搜索和票务小游戏尚未实现。
